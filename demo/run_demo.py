            ...
            messages = [
                ...
                },
                {
                    ...
                    "content": (
                        ...
                        "2. Se `response` contiene {'schema': ..., 'data': ...} (pattern dbhub a due step):\n"
                        "   - usa response['data'] per estrarre le righe SQL\n"
                        "   - puoi chiamare altri tool dall'elenco (es. generate_pie_chart) per visualizzare i dati\n"
                        "3. Per generate_pie_chart il parametro `data` DEVE essere una lista di dict con chiavi\n"
                        "   'category' e 'value' (NON 'label'!): es. [{'category': 'Diabete', 'value': 5}, ...]\n"
                        "   Chiama: call_mcp_tool('mcp-server-chart', 'generate_pie_chart',\n"
                        "           {'data': [{\"category\": r['col1'], \"value\": int(r['col2'])} for r in rows],\n"
                        "            'title': '...'})\n"
...
async def handle_prompt(state: AppState, prompt: str) -> str:
    ...
    code_raw = state.agent.code_generator.generate_from_prompt(
        ...
        user_content=(
            ...
            "from client.mcp_client import ask_llm\n"
            "schema = search_objects(object_type='table', detail_level='full')  # scopre tabelle e colonne (detail_level: 'names'|'summary'|'full')\n"
            "data = execute_sql(sql='SELECT ... FROM <tabella_dallo_schema> ...')  # usa la tabella/colonne trovate nello schema\n"
            "response = {'schema': schema, 'data': data}\n"
            "display_code = ask_llm(\n"
            f"    user_prompt={prompt!r},\n"
            "    response_data=json.dumps(response, ensure_ascii=False)[:4000],\n"
            "    available_tools=_available_tools_json,\n"
